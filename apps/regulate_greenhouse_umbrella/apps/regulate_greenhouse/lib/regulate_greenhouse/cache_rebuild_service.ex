defmodule RegulateGreenhouse.CacheRebuildService do
  @moduledoc """
  ...
  require Logger
  alias RegulateGreenhouse.Projections.Handlers.{
  ...
  def rebuild_cache do
    ...
    try do
      ...
      Logger.debug("CacheRebuildService: Calling API.get_streams with store_id: #{inspect(@store_id)}")
      ...
      case API.get_streams(@store_id) do
        {:ok, streams} when is_list(streams) ->
          ...
          Logger.info("CacheRebuildService: Cache rebuild completed successfully in #{duration_ms}ms")
          Logger.info("CacheRebuildService: Final stats: #{inspect(final_stats)}")
          
          {:ok, final_stats}
          
        {:ok, streams} ->
          Logger.error("CacheRebuildService: API.get_streams returned non-list: #{inspect(streams)}")
      ...
      e -> 
        ...
        Logger.error("CacheRebuildService: Error stacktrace: #{Exception.format_stacktrace(__STACKTRACE__)}")
  ...
  def rebuild_cache_for_streams(stream_ids) when is_list(stream_ids) do
    Logger.info("CacheRebuildService: Starting partial cache rebuild for streams: #{inspect(stream_ids)}")
    start_time = System.monotonic_time(:millisecond)
      ...
      Enum.each(stream_ids, fn stream_id ->
        ...
        if greenhouse_id do
          case Cachex.del(@cache_name, greenhouse_id) do
            {:ok, _} -> :ok
            error -> 
              Logger.warning("CacheRebuildService: Failed to clear cache for #{greenhouse_id}: #{inspect(error)}")
          end
        end
      end)
  ...
  def get_rebuild_info do
    case API.get_streams(@store_id) do
      {:ok, streams} ->
        stream_info = Enum.map(streams, fn stream_id ->
            case API.get_version(@store_id, stream_id) do
              {:ok, version} when version >= 0 -> %{stream_id: stream_id, event_count: version + 1}
              {:ok, -1} -> %{stream_id: stream_id, event_count: 0}
              {:error, _} -> %{stream_id: stream_id, event_count: 0}
            end
        end)
          ...
          case process_single_stream(stream_id) do
            {:ok, stream_stats} ->
              %{
                ...
                greenhouses_created: acc_stats.greenhouses_created + stream_stats.greenhouses_created,
                errors: acc_stats.errors ++ stream_stats.errors
              }
              
            {:error, reason} ->
              Logger.error("CacheRebuildService: Failed to process stream #{stream_id}: #{inspect(reason)}")
              %{acc_stats | errors: acc_stats.errors ++ [{stream_id, reason}]}
          end
        rescue
          e ->
            Logger.error("CacheRebuildService: Exception processing stream #{stream_id}: #{inspect(e)}")
            Logger.error("CacheRebuildService: Exception stacktrace: #{Exception.format_stacktrace(__STACKTRACE__)}")
  ...
  defp process_single_stream(stream_id) do
    ...
    case greenhouse_id do
      nil ->
        Logger.warning("CacheRebuildService: Cannot extract greenhouse ID from stream #{stream_id}")
      ...
      _ ->
        case API.get_version(@store_id, stream_id) do
          {:ok, latest_version} when latest_version >= 0 ->
            ...
            Logger.info("CacheRebuildService: Stream #{stream_id} has latest_version: #{latest_version}")
          ...
          {:error, reason} ->
            Logger.error("CacheRebuildService: Failed to get version for stream #{stream_id}: #{inspect(reason)}")
            {:error, reason}
        end
    end
  end
  ...
  defp read_events_in_batches(stream_id, greenhouse_id, total_events, current_version, acc_stats) do
    ...
    if current_version >= total_events do
      ...
      Logger.debug("CacheRebuildService: Reading batch from #{stream_id}, start_version: #{current_version}, count: #{batch_size}")
      ...
      case API.get_events(@store_id, stream_id, current_version, batch_size, :forward) do
        {:ok, events} ->
          ...
          read_events_in_batches(stream_id, greenhouse_id, total_events, next_version, combined_stats)
          
        {:error, reason} ->
          Logger.error("CacheRebuildService: Failed to get events batch for #{stream_id}: #{inspect(reason)}")
          {:error, reason}
      end
    end
  end
  ...
  defp process_event_batch(events, greenhouse_id) do
    events
    |> Enum.reduce(%{events_processed: 0, events_failed: 0, greenhouses_created: 0, errors: []}, fn event, stats ->
      case process_single_event(event, greenhouse_id) do
        ...
        {:error, reason} ->
          ...
          %{stats | 
            events_failed: stats.events_failed + 1,
            errors: stats.errors ++ [reason]
          }
      end
    end)
  end
  ...
  defp process_single_event(event, greenhouse_id) do
    ...
    {event_type, normalized_event} = case event do
  ...
  # Extract greenhouse ID from stream name (assuming format like "regulate_greenhouse_greenhouse1")
  defp extract_greenhouse_id(stream_id) when is_binary(stream_id) do
    try do
      case String.split(stream_id, "_") do
        ...
        _ ->
          Logger.warning("CacheRebuildService: Stream #{stream_id} doesn't match expected pattern")
          nil
      end
    rescue
      e ->
        Logger.error("CacheRebuildService: Error extracting greenhouse_id from #{stream_id}: #{inspect(e)}")
        nil
    end
  end
