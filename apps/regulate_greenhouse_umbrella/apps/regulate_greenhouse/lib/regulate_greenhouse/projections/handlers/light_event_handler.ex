defmodule RegulateGreenhouse.Projections.Handlers.LightEventHandler do
  @moduledoc """
  ...
  alias RegulateGreenhouse.Events.{LightSet, LightMeasured}
  ...
  def handle_event(%{data: data, event_type: "light_measured:v1"}, greenhouse_id) do
    ...
    light_value = case data do
      %{light: light} -> light
      %{"light" => light} -> light
      %RegulateGreenhouse.Events.LightMeasured{light: light} -> light
      _ -> "unknown"
    end
  ...
  def handle_event(event, greenhouse_id) do
    Logger.warning("LightEventHandler: Unhandled event type #{event.event_type} for greenhouse #{greenhouse_id}")
    :ok
  end
  ...
  defp update_greenhouse_read_model(greenhouse_id, update_fn, event_type) do
    require Logger
    Logger.debug("LightEventHandler: Updating read model for #{greenhouse_id}, event_type: #{event_type}")
    
    case Cachex.get(@cache_name, greenhouse_id) do
      {:ok, nil} ->
        Logger.warning("LightEventHandler: Greenhouse #{greenhouse_id} not found in cache for #{event_type}, will retry...")
        ...
        case Cachex.get(@cache_name, greenhouse_id) do
          {:ok, nil} ->
            Logger.error("LightEventHandler: Greenhouse #{greenhouse_id} still not found in cache for #{event_type} after retry")
            {:error, :not_found}
          {:ok, read_model} ->
            Logger.info("LightEventHandler: Found greenhouse #{greenhouse_id} in cache on retry for #{event_type}")
            do_update_read_model(greenhouse_id, read_model, update_fn, event_type)
          {:error, reason} = error ->
            Logger.error("LightEventHandler: Cache error for #{greenhouse_id} on retry: #{inspect(reason)}")
            error
        end
  ...
  defp do_update_read_model(greenhouse_id, read_model, update_fn, event_type) do
    ...
    case Cachex.put(@cache_name, greenhouse_id, updated_model) do
      ...
      error ->
        Logger.error("LightEventHandler: Failed to cache updated greenhouse #{greenhouse_id}: #{inspect(error)}")
        raise "Cache operation failed: #{inspect(error)}"
    end
    
    # Publish update for real-time UI updates
    case Phoenix.PubSub.broadcast(
      RegulateGreenhouse.PubSub, 
      "greenhouse_updates", 
      {:greenhouse_updated, updated_model, event_type}
    ) do
      :ok ->
        Logger.debug("LightEventHandler: Successfully published #{event_type} for #{greenhouse_id}")
      error ->
