defmodule RegulateGreenhouse.Infrastructure.HealthChecker do
  @moduledoc """
  ...
  alias RegulateGreenhouse.Infrastructure.{SubscriptionManager, CircuitBreaker, EventDeduplicator}
  alias RegulateGreenhouse.CacheService
  ...
  def init(opts) do
    ...
    Process.send_after(self(), :health_check, 1000)  # Start after 1 second
  ...
  defp perform_health_check(state) do
    ...
    if overall_health.status != :healthy do
      Logger.warning("HealthChecker: System health is #{overall_health.status}: #{overall_health.message}")
    else
  ...
  defp check_event_store do
    try do
      ...
      case RegulateGreenhouse.CacheService.list_greenhouses() do
        greenhouses when is_list(greenhouses) ->
          %{status: :healthy, message: "Event store accessible", details: %{greenhouse_count: length(greenhouses)}}
        error ->
          %{status: :unhealthy, message: "Event store query failed", details: %{error: error}}
      end
    rescue
      error ->
        %{status: :unhealthy, message: "Event store connection failed", details: %{error: inspect(error)}}
    end
  end
  ...
  defp check_subscriptions do
    try do
      ...
      failed_count = Enum.count(status, fn {_name, sub_status} ->
      ...
      cond do
        ...
        failed_count == 0 ->
          %{status: :healthy, message: "All subscriptions healthy", details: %{total: total_count, failed: 0}}
        
        failed_count < total_count ->
          %{status: :warning, message: "Some subscriptions failed", details: %{total: total_count, failed: failed_count}}
        
        true ->
          %{status: :unhealthy, message: "All subscriptions failed", details: %{total: total_count, failed: failed_count}}
      end
    rescue
      error ->
        %{status: :unhealthy, message: "Subscription manager unavailable", details: %{error: inspect(error)}}
    end
  end
  ...
  defp check_cache_service do
    try do
      ...
      error ->
        %{status: :unhealthy, message: "Cache service unavailable", details: %{error: inspect(error)}}
    end
  end
  ...
  defp check_event_deduplicator do
    try do
      ...
      error ->
        %{status: :unhealthy, message: "Event deduplicator unavailable", details: %{error: inspect(error)}}
    end
  end
  ...
  defp check_circuit_breakers do
    ...
    cb_status = Enum.map(circuit_breakers, fn cb_name ->
      try do
        ...
        error ->
          {cb_name, %{status: :unhealthy, message: "Circuit breaker check failed", details: %{error: inspect(error)}}}
      end
    end) |> Map.new()
    
    overall_cb_status = if Enum.any?(cb_status, fn {_name, status} -> status.status == :unhealthy end) do
      :unhealthy
    else
      :healthy
    end
  ...
  defp check_process_count do
    try do
      ...
      error ->
        %{status: :unhealthy, message: "Process count check failed", details: %{error: inspect(error)}}
    end
  end
  ...
  defp determine_overall_health(health_checks) do
    ...
    critical_issues = Enum.filter(critical_components, fn component ->
      Map.get(health_checks, component, %{}).status == :unhealthy
    end)
    
    warning_issues = Enum.filter(health_checks, fn {_component, status} ->
  ...
  defp generate_alerts(health_checks, existing_alerts, timestamp) do
    ...
    new_alerts = Enum.flat_map(health_checks, fn {component, status} ->
