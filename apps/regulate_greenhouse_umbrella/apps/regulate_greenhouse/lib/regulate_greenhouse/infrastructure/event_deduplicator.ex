defmodule RegulateGreenhouse.Infrastructure.EventDeduplicator do
  @moduledoc """
  ...
  @default_ttl :timer.minutes(10)  # 10 minutes
  @cleanup_interval :timer.minutes(5)  # 5 minutes
  ...
  def is_duplicate?(event_id) do
    case Cachex.get(@cache_name, event_id) do
      {:ok, nil} -> false
      {:ok, _} -> true
      {:error, _} -> false  # Assume it's new if cache error
    end
  end
