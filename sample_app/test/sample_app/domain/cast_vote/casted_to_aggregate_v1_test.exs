defmodule SampleApp.Domain.CastVote.CastedToAggregateV1Test do
  use ExUnit.Case, async: true
  
  alias SampleApp.Domain.CastVote.{EventV1, CastedToAggregateV1}
  alias SampleApp.Aggregate
  
  describe "CastedToAggregateV1 aggregate state update" do
    setup do
      # Create a test event
      event = %EventV1{
    ...
    test "✅ SELF-CONTAINED: only uses event data and current aggregate state", %{event: event, initial_poll: initial_poll} do
      ...
      updated_poll = CastedToAggregateV1.apply(initial_poll, event)
    ...
    test "handles multiple votes from different voters", %{initial_poll: initial_poll} do
      ...
      poll_after_event1 = CastedToAggregateV1.apply(initial_poll, event1)
      poll_after_event2 = CastedToAggregateV1.apply(poll_after_event1, event2)
      final_poll = CastedToAggregateV1.apply(poll_after_event2, event3)
    ...
    test "overwrites previous vote from same voter", %{initial_poll: initial_poll} do
      ...
      # Apply the event
      updated_poll = CastedToAggregateV1.apply(poll_with_vote, event)
    ...
    test "handles empty votes map correctly", %{event: event} do
      ...
      poll_with_no_votes = %Aggregate{
        poll_id: "empty-poll",
        votes: %{}
      }
      
      # Apply the event
      updated_poll = CastedToAggregateV1.apply(poll_with_no_votes, event)
