defmodule SampleApp.Domain.InitializePoll.InitializedToAggregateV1Test do
  use ExUnit.Case, async: true
  
  alias SampleApp.Domain.InitializePoll.{EventV1, InitializedToAggregateV1}
  alias SampleApp.Aggregate
  
  describe "InitializedToAggregateV1 aggregate state handler" do
    setup do
      # Create a test event
      event = %EventV1{
    ...
    test "✅ IMMUTABLE: creates new state without mutating input", %{event: event, initial_state: initial_state} do
      # Apply the event
      new_state = InitializedToAggregateV1.apply(initial_state, event)
      
      # Verify original state was not modified
      assert initial_state == %Aggregate{}
    ...
    test "handles event version correctly", %{initial_state: initial_state} do
      ...
      # Apply the events
      state_v1 = InitializedToAggregateV1.apply(initial_state, event_v1)
      state_v2 = InitializedToAggregateV1.apply(initial_state, event_v2)
