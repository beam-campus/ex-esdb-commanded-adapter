defmodule SampleApp.StartExpirationCountdown.CommandV1 do
  @moduledoc """
  ...
  def validate!(%__MODULE__{} = command) do
    case valid?(command) do
      ...
      {:error, :poll_id_required} ->
        raise ArgumentError, "poll_id is required"

      {:error, :expires_at_required} ->
        raise ArgumentError, "expires_at is required"

      {:error, :expiration_must_be_future} ->
        raise ArgumentError, "expiration must be in the future"
    end
  end
