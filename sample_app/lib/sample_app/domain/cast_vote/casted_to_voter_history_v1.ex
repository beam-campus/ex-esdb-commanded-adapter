  ...
  def handle(%VoteCastedEvent{} = event, _metadata) do
    ...
    update_func = fn
      nil ->
        ...
        updated_history = VoterHistory.add_vote(new_history, event.poll_id, event.option_id, event.voted_at)
        {nil, updated_history}
      %VoterHistory{} = history ->
        # Update existing voter history
        updated_history = VoterHistory.add_vote(history, event.poll_id, event.option_id, event.voted_at)
    ...
    case Cachex.get_and_update(:voter_histories, event.voter_id, update_func) do
      ...
      {:error, reason} ->
        Logger.error("❌ Failed to update voter history for voter: #{event.voter_id}, reason: #{inspect(reason)}")
        {:error, reason}
    end
  end
end
